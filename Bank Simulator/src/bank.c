...
int bank(int atm_out_fd[], Command *cmd, int *atms_remaining) {
  ...
  if(check_valid_atm(i) != SUCCESS)//Used to check that the ATM ID unpacked from the received command is a valid ATM.
  {
    return ERR_UNKNOWN_ATM;
  }
  else 
  {
    switch (c)//Used to perform different actions for each of the different command types: CONNECT, EXIT, DEPOSIT, WITHDRAW, TRANSFER, and BALANCE.
    {
      case CONNECT://If the command type is CONNECT.
        MSG_OK(&bankcmd, 0, f, t, a);//Used to create an OK message type.
        result = checked_write(atm_out_fd[i], &bankcmd, MESSAGE_SIZE);//Used the checked_write function to write to the correct ATM.
        break;
      case EXIT://If the command type is EXIT.
        (*atms_remaining) -= (*atms_remaining);//Atms remaining decremented here.
        MSG_OK(&bankcmd, 0, f, t, a);//Used to create an OK message type.
        result = checked_write(atm_out_fd[i], &bankcmd, MESSAGE_SIZE);//Used the checked_write function to write to the correct ATM.
        break;
      case DEPOSIT://If the command type is DEPOSIT.
        if(check_valid_account(t) != SUCCESS)//Check the to account using the check_valid_account function to make sure that it is a valid account.
        {
          MSG_ACCUNKN(&bankcmd, i, t);//To indicate that the account was invalid.
          result = checked_write(atm_out_fd[i], &bankcmd, MESSAGE_SIZE);//Used the checked_write function to write to the correct ATM.
          break;
        }
        else
        {
          accounts[t] += a;//Updating the account with the amount.
          MSG_OK(&bankcmd, 0, f, t, a);//Used to create an OK message type.
          result = checked_write(atm_out_fd[i], &bankcmd, MESSAGE_SIZE);//Used the checked_write function to write to the correct ATM.
          break;
        } 
      case WITHDRAW://If the command type is WITHDRAW.
        if(check_valid_account(f) != SUCCESS)//Check the from account using the check_valid_account function to make sure that it is a valid account.
        {
          MSG_ACCUNKN(&bankcmd, i, t);//To indicate that the account was invalid.
          result = checked_write(atm_out_fd[i], &bankcmd, MESSAGE_SIZE);//Used the checked_write function to write to the correct ATM.
          break;
        }
        else if(accounts[f] < a)//Check the amount being transferred is available in the from account.
        {
        ...
        {
          accounts[f] -= a;//Updating the account.
          MSG_OK(&bankcmd, 0, f, t, a);//Used to create an OK message type.
          result = checked_write(atm_out_fd[i], &bankcmd, MESSAGE_SIZE);//Used the checked_write function to write to the correct ATM.
          break;
        }   
      case TRANSFER://If the command type is TRANSFER.
        if(check_valid_account(f) != SUCCESS)//Check the from account using the check_valid_account function to make sure that it is a valid account.
        {
          MSG_ACCUNKN(&bankcmd, i, t);//To indicate that the account was invalid.
          result = checked_write(atm_out_fd[i], &bankcmd, MESSAGE_SIZE);//Used the checked_write function to write to the correct ATM.
        ...
        else if(check_valid_account(t) != SUCCESS)//Check the to account using the check_valid_account function to make sure that it is a valid account.
        {
          MSG_ACCUNKN(&bankcmd, i, t);//To indicate that the account was invalid.
          result = checked_write(atm_out_fd[i], &bankcmd, MESSAGE_SIZE);//Used the checked_write function to write to the correct ATM.
          break;
        ...
        {
          accounts[f] -= a;//Updating the account.
          accounts[t] += a;//Updating the account.
          MSG_OK(&bankcmd, 0, f, t, a);//Used to create an OK message type.
          result = checked_write(atm_out_fd[i], &bankcmd, MESSAGE_SIZE);//Used the checked_write function to write to the correct ATM.
          break;
        } 
      case BALANCE://If the command type is BALANCE.
        if(check_valid_account(f) != SUCCESS)//Check the from account using the check_valid_account function to make sure that it is a valid account.
        {
          MSG_ACCUNKN(&bankcmd, i, t);//To indicate that the account was invalid.
          result = checked_write(atm_out_fd[i], &bankcmd, MESSAGE_SIZE);//Used the checked_write function to write to the correct ATM.
      ...
      default:
        error_msg(ERR_UNKNOWN_CMD, "Command received does not match any of the expected commands above");//We send an error message.
        result = ERR_UNKNOWN_CMD;
        break;
    }   
  return result;//Result returned at the end.
  }
}
